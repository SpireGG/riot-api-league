...
class LeagueAPI extends BaseAPI
{
    ...
    public const
        ...
        SET_INTERIM = 'SET_INTERIM', /** Used to set whether or not your application is in Interim mode (Tournament STUB endpoints) **/
        ...
        SET_DATADRAGON_INIT = 'SET_DATADRAGON_INIT', /** Specifies whether or not the DataDragonAPI is to be initialized by this library **/
    ...
    public function getStaticItems(string $locale = 'en_US', string $version = null): StaticData\StaticItemListDto
    {
        ...
        // Create missing data
        array_walk($result['data'], static function (&$d, $k) {
            $d['id'] = $k;
        });
        $this->result_data = $result;
    ...
    public function createTournamentCodes(int $tournament_id, int $count, TournamentCodeParameters $parameters): array
    {
        ...
        if (!in_array($parameters->pickType, self::TOURNAMENT_ALLOWED_PICK_TYPES, true)) {
        ...
        if (!in_array($parameters->mapType, self::TOURNAMENT_ALLOWED_MAPS, true)) {
        ...
        if (!in_array($parameters->spectatorType, self::TOURNAMENT_ALLOWED_SPECTATOR_TYPES, true)) {
        ...
        $data = json_encode($parameter_array, JSON_THROW_ON_ERROR);
     ...
     */
    public function editTournamentCode(string $tournament_code, TournamentCodeUpdateParameters $parameters): void
    {
        if ($this->getSetting(self::SET_INTERIM, false)) {
        ...
        $data = json_encode($parameters, JSON_THROW_ON_ERROR);
    ...
    public function createTournamentProvider(ProviderRegistrationParameters $parameters): int
    {
        ...
        if (!in_array(strtolower($parameters->region), self::TOURNAMENT_ALLOWED_REGIONS, true)) {
        ...
        $data = json_encode($parameters, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES);
